...
link_librarys  := $(link_opencv) rknn_api $(link_sys)
...
cpp_compile_flags := -std=$(stdcpp) -w -g -O3 -fPIC -fopenmp -pthread
cu_compile_flags  := -std=$(stdcpp) -w -g -O3 $(cuda_arch) -Xcompiler "$(cpp_compile_flags)"
